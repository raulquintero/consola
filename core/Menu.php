...
//if (in_array($sub['privilegio_id'], $_SESSION['privilegios']))
{
         ...
         if ($level==0)
        {
            ...
            echo "<li class=\"$active\" ><a href=\"".$sub['path_option']."\" >".ucwords($sub['menu'])."</a></li>";
            }
        }
        else
        {
            ...
            list( $cuantos_subs ) = $this->database->get_row( $query2 );
            if ($cuantos_subs>0)
                echo "<li class='dropdown-submenu'><a href='#' data-toggle='dropdown' class='dropdown-toggle'><b>".ucwords($sub['menu'])."</b></a><ul  class=\"dropdown-menu\">";
            else{
        ...
        echo '</li>';
            ...
            // creacion de privilegios por primera vez, no se vuelva a ejecutar
            if (0){
...
public function showListCategory($parent_id,$level=0) {
    ...
    foreach ($subs as $sub) {
    	 if ($level==0)
        {
        	...
        	echo "<li class=\"\"><a href=\"/mantenimiento/menu/".$sub['menu_id']."\">".ucfirst($sub['menu'])."</a>";
        }
        else
        {
        	echo "<li><a tabindex=\"-1\" href=\"/mantenimiento/menu/".$sub['menu_id']."\">".ucfirst($sub['menu'])."</a></li>";
        }
...
public function showCategory($parent_id=0,$level=0) {
    ...
    $query = "SELECT menu, menu_id , parent_id, path_option,parametros,position,privilegio_id FROM menu  WHERE  parent_id=".$parent_id." AND deleted=FALSE ORDER BY position ASC";
    // $res = mysql_query($query) or die($query);
    $subs = $this->database->get_results($query);
    $cuantos=count ($subs);
    if($cuantos == 0) return;
    foreach ($subs as $sub) {
    	...
    	$n_subs = count($n_subs);
        	echo "<td>".$sub['menu_id']."</td><td><span class='visible-print'>".$sub['menu']."</span><a class='hidden-print' href='/mantenimiento/menu/".$sub['menu_id']."'>".$sub['menu']."</a></td>";
            if ($n_subs)    
                echo "<td> <i class=\"glyphicon-list\"></i> SubMenu </td>";
                else echo "<td>".$sub['path_option']."</td>";

        	echo "<td>".Crypto64::decode($sub['parametros'])."</td>";
        	echo "<td>".$sub['position']."</td>

        	<td>".$sub['privilegio_id']."</td>
            <td>
            <a class='hidden-print' href=\"#modal-window\" data-toggle=\"modal\"  onclick='showModal(\"editarmenu\",".$sub['menu_id'].")'><button class=\"btn btn-success\">Editar <!--i class=\"icon-plus icon-white\"></i--></button></a></td>
            ";

            if (!$n_subs)
            echo "<td class='hidden-print'>
        		<a class='hidden-print' href='/mantenimiento/borrarmenu/".$sub['menu_id']."/$parent_id' >
...
public function showCategoryDeleted($parent_id=0,$level=0) {
    ...
    foreach ($subs as $sub) {
        ...
        $n_subs = count($n_subs);
            ...
            <td>".$sub['privilegio_id']."</td>
            <td>
            <a class=hidden-print href='/mantenimiento/restaurarmenu/".$sub['menu_id']."/$parent_id'><button class=\"btn btn-success\">Restaurar <!--i class=\"icon-plus icon-white\"></i--></button></a></td>
            ";

            if (!$n_subs){
            echo "<td>
                <a class=hidden-print href='/mantenimiento/removermenu/".$sub['menu_id']."/$parent_id' >
...
//    echo "menu_id:".$menu_id;
    $query = "SELECT menu, menu_id , parent_id, path_option FROM menu  "
            . "WHERE  menu<>'*div*' AND deleted=FALSE AND parent_id='".$parent_id."' ORDER BY position ASC";
    // $res = mysql_query($query) or die($query);
    $subs = $this->database->get_results($query);
    $cuantos=count ($subs);
    if($cuantos == 0) return;
 ...
 $tab="";
    ...
    foreach ($subs as $sub) {
         if ($level==0)
        {
            ...
            if ($sub['menu_id']==$menu_id) $selected=" selected "; else $selected = NULL;
                    echo "<option value=".$sub['menu_id']." $selected>$tab".ucfirst($sub['menu'])." </option>";
        }
...
public function getParentId($menu_id){
    ...
    list($parent_id) = $this->database->get_row($query);
    //$query = "SELECT menu_id from menu  WHERE menu_id=$parent_id";
    //list($parent_id) = $database->get_row($query);
    return $parent_id;
}

public function getNameParentId($menu_id){

    $query = "SELECT menu FROM menu WHERE menu_id=$menu_id";
    list($parent_id) = $this->database->get_row($query);
    //$query = "SELECT menu_id from menu  WHERE menu_id=$parent_id";
    //list($parent_id) = $database->get_row($query);
    return $parent_id;
...
*/
public function editCategory($parent_id,$menu_name,$path_option,$parametros,$menu_id,$position){
...
public function insertCategory($parent_id,$menu_name,$path_option,$parametros){
