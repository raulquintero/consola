...
elseif 	($v = Router::request('/logout','get','public'))						{$response = ($v['security']) ? LoginController::logOut("none",$v):array('security'=>TRUE);} 
elseif 	($v = Router::request('/lg/error','get','public'))						{$response = ($v['security']) ? LoginController::LoginError("loginEmail",$v):array('security'=>TRUE);} 
elseif 	($v = Router::request('/login/{key}','get','public'))					{$response = ($v['security']) ? LoginController::checkPassword("checkPassword",$v):array('security'=>FALSE);} 
...
elseif 	($v = Router::request('/pos','get','ro2'))								{$response = ($v['security']) ? PosController::pos("xpos",$v):array('security'=>FALSE);}  

elseif 	($v = Router::request('api/inicio/ordenes_nuevas','get','ro0'))		{$response = ($v['security']) ? HomeController::ordenesNuevas("api",$v):array('security'=>FALSE);}  
